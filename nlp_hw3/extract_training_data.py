...
'''
COMS 4705
This file extracts two numpy matrices representing input output pairs.
Completed by Manxueying Li
'''
...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        # sigma = state.stack  #stack
        # beta = state.buffer  #buffer
        vec = []
        for curr in [state.stack, state.buffer]:
            for j in range(3):
                if j >= len(curr):
                    vec.append(4) # NULL = 4
                else:
                    ind = curr[-j-1] #get index from statck/buffer
                    if ind == 0:
                        vec.append(3)
                    elif pos[ind] == 'CD':
                        vec.append(0) # CD = 0
                        ...
                        vec.append(1) # NNP = 1
                    elif words[ind] in self.word_vocab:
                        vec.append(self.word_vocab[words[ind]])
                    else:
                        vec.append(2) # UNK = 0
    ...
    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        onehot = keras.utils.to_categorical(self.output_labels[output_pair], num_classes=91, dtype="int8")
        return onehot.reshape(91)
